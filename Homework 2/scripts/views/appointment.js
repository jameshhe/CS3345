(app => {
    ...
    app.appointmentView = {
        load: (tutorId, day) => {
            selected.push(tutorId, day)
            app._changeView('appointmentView')
        },
        save: () => {
            const section = document.querySelector('#appointmentView')
            let form = section.querySelector('form')
            let name = form.querySelector('#student_name')
            let notes = form.querySelector('#student_notes')
            let newAppointment = [...selected, name.value, notes.value]
            app.scheduler.saveAppointment(newAppointment)
            form.reset()
